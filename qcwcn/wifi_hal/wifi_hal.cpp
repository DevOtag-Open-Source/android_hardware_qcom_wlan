...
wifi_error wifi_initialize(wifi_handle *handle)
{
    ...
    *handle = (wifi_handle) info;
    ...
    ALOGV("Initialized Wifi HAL Successfully; vendor cmd = %d Supported"
            " features : %llx", NL80211_CMD_VENDOR, info->supported_feature_set);
    ...
    *set = 0;
    ...
    if (ret != WIFI_SUCCESS) {
        *set = info->supported_feature_set;
        ALOGV("Supported feature set acquired at initialization : %llx", *set);
    } else {
        info->supported_feature_set = *set;
        ALOGV("Supported feature set acquired : %llx", *set);
    }
    return WIFI_SUCCESS;
}
